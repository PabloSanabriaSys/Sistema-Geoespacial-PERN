...
import { getCantones, getCantonUsers, getClients } from '../../api/client';
...
export default function UbicationClientCantones() {
    ...
    const fetchClientLocations = async () => {
        try {
            ...
            const response2 = await getCantones();
    ...
    const changeSection = async (cantonName) => {
        try {
            const response = await getCantonUsers(cantonName);
            const data = response.data;
            const validLocations = filtrarClients(data.usuarios);
            const canton = data.canton;
            setGeojsonData(canton);
            setClientLocations(validLocations);
