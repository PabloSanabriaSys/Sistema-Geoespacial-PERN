...
import { getClients, getMunicipios, getMunicipioUsers } from '../../api/client';
...
export default function UbicationClientMunicipios() {
    ...
    const fetchClientLocations = async () => {
        try {
            ...
            const response2 = await getMunicipios();
    ...
    const changeSection = async (municipioName) => {
        try {
            const response = await getMunicipioUsers(municipioName);
            const data = response.data;
            const validLocations = filtrarClients(data.usuarios);
            const municipio = data.municipio;
            setGeojsonData(municipio);
            setClientLocations(validLocations);
    ...
    const onEachSection = async (section, layer) => {
        if (section.properties != undefined && section.properties.nombre != undefined) {
            const sectionName = section.properties.nombre;
    ...
    return (
        <div className="sm:px-20 py-10 px-2 overflow-hidden flex flex-col justify-center" style={{ height: "100vh" }}>
            <Title title="UBICACION DE LOS CLIENTES" subtitle="" titleSize="sm:text-6xl text-4xl" subtitleSize="sm:text-6xl text-4xl mb-0" />
